...
In the \autoref{fig:max_average_min}, we show the \emph{Box} plot for three different possible cases that might appear when running the simulation script. The first box (the blue box) represents the runtime when running the simulation script with explicity calling the PyNESTML module and mannualy generating the code for the neuron model. The second box, the orange box represents the runtime when running the simulation with the JIT module with the neuron model being already built in the libaray. Lastly, the green box represents the runtime when running the simulation with the JIT module and with the neuron model being solely in a NESTML format. All the three experiences share the same network topology and only differ from the way how instances from the model are loaded and created.
